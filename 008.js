...
function largestProductinaSeries(series, digitsInSeries) { 
    let [largestProduct] = series.reduce(([max, numbers], currentNumber) => {
        if(numbers.length === digitsInSeries) {
            numbers.shift();
        }
        numbers.push(currentNumber);
        let currentResult = multiplication(numbers);
        return [ Math.max(max, currentResult), numbers];
    }, [ 0, [] ]);
